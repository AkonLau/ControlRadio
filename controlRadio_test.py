...
def main(args):
    ...
    # train_dataloader = DataLoader(train_dataset, num_workers=4, batch_size=args.batch_size, shuffle=False)
    val_dataloader = DataLoader(val_dataset, num_workers=4, batch_size=args.test_batch_size, shuffle=False)

    # self.log_images_kwargs
    log_images_kwargs = {"unconditional_guidance_scale": args.ugs, "sample": args.sample,
                         "test_simulation": args.simulation}
    logger = ImageLogger(batch_frequency=args.logger_freq, log_images_kwargs=log_images_kwargs)

    trainer = pl.Trainer(default_root_dir=args.save_dir, gpus=args.gpus, strategy="ddp", precision=32,
                         callbacks=[logger],
